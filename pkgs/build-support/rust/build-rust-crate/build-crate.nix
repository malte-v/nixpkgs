...
{ crateName,
  ...
  targetOverride, extraRustcOpts, verbose, colors,
  buildTests
}:
  ...
  let
    targetOpts =
      if targetOverride != null
        then [ "--target ${targetOverride}" ]
      else if (stdenv.hostPlatform != stdenv.buildPlatform)
        then [ "--target ${rust.toRustTargetSpec stdenv.hostPlatform}" "-C linker=${stdenv.hostPlatform.config}-gcc" ]
      else [];
    baseRustcOpts =
