...
let
  ...
  machine-learning = python.pkgs.buildPythonApplication {
    ...
    postPatch = ''
      ...
      # opencv in named differently, also remove development dependencies not needed at runtime
      substituteInPlace pyproject.toml \
        --replace 'opencv-python-headless = "^4.7.0.72"' "" \
        --replace 'pydantic = "^1.10.8"' "" \
        --replace 'pytest-cov = "^4.1.0"' "" \
        --replace 'ruff = "^0.0.272"' ""

      # XXX: Remove these once nix packages reach compatible versions
      substituteInPlace pyproject.toml \
        --replace 'fastapi = "^0.95.2"' 'fastapi = ">=0.95.1"' \
        --replace 'onnxruntime = "^1.15.0"' 'onnxruntime = ">=1.13.1"' \
        --replace \
    ...
    postInstall = ''
      makeWrapper ${python.interpreter} $out/bin/machine-learning \
