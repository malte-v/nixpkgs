...
let
  ...
  logCfg = optionalString (cfg.logDirectory != null)
    "log fs ${cfg.logDirectory}";

  configFile = pkgs.writeText "soju.conf" ''
    ${listenCfg}
    hostname ${cfg.hostName}
    ${tlsCfg}
    db sqlite3 ${cfg.databasePath}
    ${logCfg}
...
in
{
  ...
  options.services.soju = {
    ...
    databasePath = mkOption {
      type = types.path;
      default = "/var/lib/soju/soju.db";
      description = "Path to the SQLite database.";
    };

    logDirectory = mkOption {
      type = types.nullOr types.path;
      example = "/var/lib/soju/logs";
      description = "Path to the bouncer logs root directory.";
    };
  ...
  config = mkIf cfg.enable {
    users.users.soju = {
      description = "soju IRC bouncer user";
      isSystemUser = true;
      group = "soju";
    };
    users.groups.soju = {};
